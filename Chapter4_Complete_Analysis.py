...
This script provides a comprehensive analysis for Chapter 4 of the thesis.
...
print(f"\nTotal records: {len(sentiment_df):,}")
...
axes[0,0].axvline(sentiment_df['sentiment_score'].mean(), color='red', linestyle='--',
                  label=f'Mean: {sentiment_df["sentiment_score"].mean():.3f}')
axes[0,0].legend()
...
bars = plt.bar(range(len(models_sorted)), models_sorted['Accuracy'])
